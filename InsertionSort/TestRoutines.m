...
//    NSString *testData = [NSString stringWithFormat:@"Array Size,Elapsed Time in ms\n"];    // Write heading line to CSV file
    ...
    NSString *testData = [NSString stringWithFormat:@"Array Size,Best Time in ms,Worst Time in ms\n"];    // Write heading line to CSV file
    [self.csvFileMakerObject writeToCsvFile:testData];
    
    numTests = 100;
    
    
    //for (arraySize = 1000000; arraySize < 1000001; arraySize *= 10) {             // arraySize = 10, 100, 1000, 10000, 100000, 1000000
    ...
    for (arraySize = 1000000; arraySize <= 10000000; arraySize += 200000) {        // arraySize = 1 million, 1.2 million, 1.4 million, ..., 10 million
        ...
        NSLog(@"\nBest Time for this array size = %f; Worst Time = %f", bestTime, worstTime);
        
        NSString *testData = [NSString stringWithFormat:@"%d,%f,%f\n", arraySize, bestTime, worstTime];    // Write results for this array size to CSV File
        [self.csvFileMakerObject writeToCsvFile:testData];
        
        NSLog(@"\nEnding Tests for Array Size: %d\n", arraySize);
    }
}
...
- (void)sortTheArray {
    ...
    double elapsedTime = [self.timerObject timeElapsedInMilliseconds];      // elapsed time for this sorting test
    totalTime += elapsedTime;                                               // cumulative elapsed time for all sorting tests of this array size
    if (elapsedTime > worstTime) {
        worstTime = elapsedTime;                                            // worst (i.e., longest) time to do a sort for this array size
    }
    if (elapsedTime < bestTime) {
        bestTime = elapsedTime;                                             // best (i.e., shortest) time to do a sort for this array size
    }
...
/* Old Notes (from testing under battery power with some sort of power saving in effect):
